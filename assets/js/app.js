...
function iniciarFormRegistro() {
  ...
  $('#formRegistro').addEventListener('submit', async e => {
    e.preventDefault();
    const btn = e.submitter || $('#formRegistro button[type=submit]');
    ...
    try {
      ...
      aviso(reemplazado
        ? `Registro de ${reg.agenteNombre} para el ${fechaCorta(fecha)} actualizado.`
...
function iniciarAgentes() {
  $('#formAdmin').addEventListener('submit', async e => {
    ...
    try {
      ...
      aviso('Sesión de administrador iniciada.');
    } catch (err) {
      aviso(err.message, 'error');
    }
  });
